...
xx_var="Statistic";
format percent 8.1;
if strip(vvalue(&&var&i..)) ^= '0' AND strip(vvalue(&&var&i..)) ^= 'F';
run;
...
proc print data=&output_data. label;
%if n_byvar.>1 %then %do;
var xx_variable xx_description  xx_var_all xx_var1-xx_var&n_byvar.;
%end;
%else %if n_byvar.=1 %then %do;
var xx_variable xx_description  xx_var_all xx_var1;
%end;
run;
